...
class GUI_window(QtWidgets.QMainWindow):
    ...
    def show_image(self):
        ...
        if rat == True:
            ...
            #frame = self.YOLO_image_queue.get()#把RGB圖片拿出來用
            #detections = self.detections_queue.get()#把框框座標取出來(有可能會是空的 因為沒偵測到臉)
...
#                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), 2)
    ...
    def open_detect(self):
        if self.check == 0:
            ...
            self.cap=cv2.VideoCapture(0)
            #-------------口罩辨識 多執行續啟動----------
            Thread(target=self.video_capture, args=()).start()
            Thread(target=self.inference, args=()).start()
            Thread(target=self.drawing, args=()).start()
            #-------------------------------------------
            self.timer_camera = QtCore.QTimer()
